...
var util = require('@sap-cloud-sdk/util');
...
const _OAuthClient = class _OAuthClient {
  ...
  constructor(config) {
    ...
    this.logger = util.createLogger("btp-llm-commons:auth");
    this.config = config;
    this.httpClient = axios.create({
      baseURL: util.removeTrailingSlashes(config.oauthTokenUrl)
    });
  }
  /**
...
class BTPLLMHttpClient {
  ...
  constructor(config) {
    ...
    this.config = config;
    this.httpClient = axios.create({
      baseURL: util.removeTrailingSlashes(config.llmProxyBaseUrl)
    });
...
class BaseLLMClient {
  ...
  constructor(name, httpClient) {
    ...
    this.name = name;
    this.httpClient = httpClient != null ? httpClient : new BTPLLMHttpClient(index.BTPLLMContext.getConfig());
    this.logger = util.createLogger(`btp-llm-commons:llm-client:${name}`);
  }
  /**
   *
   * @param path The API URL path
  ...
  async request(path, data, requestConfig) {
    try {
      ...
      if (err.response) {
        ...
        throw new index.BTPLLMError(msg, err);
      } else if (err.request) {

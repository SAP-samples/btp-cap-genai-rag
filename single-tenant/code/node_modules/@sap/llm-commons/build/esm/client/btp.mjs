...
import { n as normalizeRange } from '../utils-8f77ab42.mjs';
import { BTPAI21JurassicClient } from './ai21.mjs';
import { BTPAlephAlphaLuminousClient } from './alephalpha.mjs';
import { BTPAmazonTitanClient } from './amazon.mjs';
import { BTPAnthropicClaudeClient } from './anthropic.mjs';
import { BTPCohereCommandClient } from './cohere.mjs';
import { B as BaseLLMClient } from '../base-540b2ebe.mjs';
import { BTPGooglePaLMClient } from './google.mjs';
import { BTPHuggingFaceClient } from './huggingface.mjs';
import { BTPOpenAIGPTClient } from './openai.mjs';
...
class BTPLLMProxyClient extends BaseLLMClient {
  /**
  ...
  async createTextCompletion(params, requestConfig) {
    ...
    if (deploymentId === "text-davinci-003" || deploymentId === "code-davinci-002" || deploymentId === "gpt-35-turbo" || deploymentId === "gpt-35-turbo-16k" || deploymentId === "gpt-4" || deploymentId === "gpt-4-32k") {
      ...
      throw new BTPLLMError("Cannot infer LLM model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaOpenAIGPTClient(params, requestConfig) {
    const deploymentId = params.deployment_id;
    const openaiClient = new BTPOpenAIGPTClient(this.httpClient);
    if (deploymentId === "text-davinci-003" || deploymentId === "code-davinci-002") {
      const res = await openaiClient.createCompletion(
        {
          ...
          temperature: normalizeRange(params.temperature, 0, 2),
          top_p: normalizeRange(params.topP, 0, 1),
          n: params.n,
          stop: params.stop
        },
        requestConfig
      ...
      throw new BTPLLMError("Cannot infer OpenAI GPT model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaGooglePaLMClient(params, requestConfig) {
    var _a, _b;
    const deploymentId = params.deployment_id;
    const googlePalmClient = new BTPGooglePaLMClient(this.httpClient);
    if (deploymentId === "gcp-text-bison-001") {
      const res = await googlePalmClient.predictText(
        {
          ...
          parameters: {
            ...
            temperature: normalizeRange(params.temperature, 0, 1),
            topP: normalizeRange(params.topP, 0, 1),
            topK: params.topK
          }
        },
        requestConfig
      ...
      throw new BTPLLMError("Cannot infer Google PaLM model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaAnthropicClaudeClient(params, requestConfig) {
    var _a, _b;
    const deploymentId = params.deployment_id;
    const anthropicClaudeClient = new BTPAnthropicClaudeClient(this.httpClient);
    if (deploymentId === "anthropic-claude-v1" || deploymentId === "anthropic-claude-instant-v1" || deploymentId === "anthropic-claude-v2") {
      ...
      throw new BTPLLMError("Cannot infer Anthropic Claude model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaCohereCommandClient(params, requestConfig) {
    var _a, _b, _c;
    const deploymentId = params.deployment_id;
    const cohereCommandClient = new BTPCohereCommandClient(this.httpClient);
    if (deploymentId === "cohere-command") {
      const res = await cohereCommandClient.createTextCompletion(
        {
          ...
          temperature: normalizeRange(params.temperature, 0, 5),
          p: normalizeRange(params.topP, 0.01, 0.99),
          k: params.topK,
          stop_sequences: (_b = params.stop) != null ? _b : ["\n\n"],
      ...
      throw new BTPLLMError("Cannot infer Cohere Command model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaAmazonTitanClient(params, requestConfig) {
    const deploymentId = params.deployment_id;
    const amazonTitanClient = new BTPAmazonTitanClient(this.httpClient);
    if (deploymentId === "amazon-titan-tg1-large") {
      ...
      throw new BTPLLMError("Cannot infer Amazon Titan model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaAlephAlphaLuminousClient(params, requestConfig) {
    var _a, _b;
    const deploymentId = params.deployment_id;
    const alephAlphaLuminousClient = new BTPAlephAlphaLuminousClient(this.httpClient);
    if (deploymentId === "alephalpha") {
      ...
      throw new BTPLLMError("Cannot infer AlephAlpha Luminous model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaAI21JurassicClient(params, requestConfig) {
    var _a, _b;
    const deploymentId = params.deployment_id;
    const ai21JurassicClient = new BTPAI21JurassicClient(this.httpClient);
    if (deploymentId === "ai21-j2-grande-instruct" || deploymentId === "ai21-j2-jumbo-instruct") {
      ...
      throw new BTPLLMError("Cannot infer AI21 Jurassic model from `deployment_id`");
    }
  }
  /**
  ...
  async createTextCompletionViaHuggingFaceClient(params, requestConfig) {
    const deploymentId = params.deployment_id;
    const huggingFaceClient = new BTPHuggingFaceClient(this.httpClient);
    if (deploymentId === "falcon-7b" || deploymentId === "falcon-40b-instruct" || deploymentId === "llama2-13b-chat-hf" || deploymentId === "llama2-70b-chat-hf") {
      const res = await huggingFaceClient.textGeneration(
        {
          ...
          parameters: {
            ...
            temperature: normalizeRange(params.temperature, 0, 100),
            top_p: normalizeRange(params.topP, 0, 1.00000010000001),
            top_k: params.topK
          }
        },
        requestConfig
      ...
      throw new BTPLLMError("Cannot infer HuggingFace model from `deployment_id`");
    }
  }
  /// / / / / / / / / ///
  /// CHAT COMPLETION ///
  ...
  async createEmbeddingViaOpenAIGPTClient(params, requestConfig) {
    const deploymentId = params.deployment_id;
    const openaiGPTClient = new BTPOpenAIGPTClient(this.httpClient);
    if (deploymentId === "text-embedding-ada-002-v2") {

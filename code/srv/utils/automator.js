...
class TenantAutomator {
    ...
    constructor(tenant, subdomain, custdomain = null) {
        (this.tenant = tenant), (this.subdomain = subdomain);
...
class Kyma extends TenantAutomator {
    constructor(tenant, subdomain, custdomain) {
        try {
            ...
            this.destinationName = `${process.env["HELM_RELEASE"].toUpperCase()}_${process.env[
                "KYMA_NAMESPACE"
            ].toUpperCase()}_S4HANA_CLOUD`;
        } catch (error) {
...
class CloudFoundry extends TenantAutomator {
    ...
    async createRoute() {
        try {
            await this.cfUtils.createRoute(
                this.subdomain + process.env.tenantSeparator + process.env.appName,
                process.env.appName
            );
        } catch (error) {
    ...
    async deleteRoute() {
        try {
            await this.cfUtils.deleteRoute(
                this.subdomain + process.env.tenantSeparator + process.env.appName,
                process.env.appName
            );
        } catch (error) {
